...
run_xray() {
   re_uuid=$(curl -s $REPLIT_DB_URL/re_uuid)   
    ...
    echo
    green "Currently installed official version of Xray：$RELEASE_LATEST"
    echo
    UA_Browser="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.87 Safari/537.36"
    v4=$(curl -s4m6 ip.sb -k)
    v4l=`curl -sm6 --user-agent "${UA_Browser}" http://ip-api.com/json/$v4?lang=zh-CN -k | cut -f2 -d"," | cut -f4 -d '"'`
    echo
    green "IP Shenasaee shode feli：$v4    地区：$v4l"
    echo
    blue "五大协议相关信息如下------------------------------"
echo
    yellow "1：vmess+ws+tls.，In parametrha ro copy kon to v2ray"
    echo "server address：${REPL_SLUG}.${REPL_OWNER}.repl.co"
    echo "Port：443"
    echo "uuid：$user_uuid"
    echo "Protocol：ws"
...
echo
yellow "Qr-code-khematet"
qrencode -t ansiutf8 ${replit_xray_vmess}
echo
blue "-------------------------------------------"
echo
yellow "2：vless+ws+tls.，In parametrha ro copy kon to v2ray"
    echo "server address：${REPL_SLUG}.${REPL_OWNER}.repl.co"
    echo "Port：443"
    echo "uuid：$user_uuid"
    echo "Protocol：ws"
...
blue "-------------------------------------------"
echo
yellow "3：trojan+ws+tls.，In parametrha ro copy kon to v2ray"
    echo "server address：${REPL_SLUG}.${REPL_OWNER}.repl.co"
    echo "Port：443"
    echo "password：$user_uuid"
    echo "Protocol：ws"
    echo "host/sni：${REPL_SLUG}.${REPL_OWNER}.repl.co"
...
blue "-------------------------------------------"
echo
yellow "4：shadowsocks+ws+tls.，In parametrha ro copy kon to v2ray"
    echo "server address：${REPL_SLUG}.${REPL_OWNER}.repl.co"
    echo "Port：443"
    echo "password：$user_uuid"
    ...
    blue "-------------------------------------------"
    echo
    yellow "5：socks+ws+tls.，In parametrha ro copy kon to v2ray"
    echo "server address：${REPL_SLUG}.${REPL_OWNER}.repl.co"
    echo "Port：443"
    echo "用户名：$user_uuid"
    echo "password：$user_uuid"
...
echo
echo ".update log：
23.1.20Update: Integrated automatic wake-up function every 10 minutes
23.1.27Update: One key and five protocols coexist, the domain name page shows ا
Translate By: Peyman

视频教程：https://www.youtube.com/@P_Tech
